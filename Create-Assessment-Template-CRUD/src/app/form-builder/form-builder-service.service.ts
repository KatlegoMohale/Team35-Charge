...
export class FormBuilderService {
  ...
  generateHtml() {
    let html = `<!doctype html>
      <html>
        <head>
        <title>Create Assessment Template</title>
        <!--[if !mso]>-->
  ...
  parseComponentSettings(component: ElementInterface) {
    ...
    if (
      ...
      if (spacerEl.height > 0) {
        styles += `height: ${spacerEl.height}px;`;
      } else {
        styles += `height: auto;`;
      }
    } else {
      if (
        ...
        styles += `height: ${component?.componentSettings?.height}px;`;
      } else {
        styles += `height: auto;`;
      }
    }
  ...
  parseButtonComponent(component: ElementInterface) {
    ...
    if (el.link_to) {
      ...
      $div += `<button type="button" style="${buttonStyles}">
        ${el.content}
      </button>`;
    }
